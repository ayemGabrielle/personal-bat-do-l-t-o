...
import '../screens/Edit-MVfile-Screen.dart';
import 'package:lto_app/screens/mvFile_screen.dart';
...
void _fetchMVFiles() async {
  ...
  if (connectivityResult == ConnectivityResult.none) {
    print("⚠️ No internet connection. Loading MV files from local storage...");
...
Future<void> deleteMVfile(MVFile mvFile) async {
            ...
            onPressed: () async {
              ...
              if (connectivityResult == ConnectivityResult.none) {
                ...
                try {
                  ...
                  print("Failed to delete online, adding to pending queue: $e");
  ...
  Future<void> _saveMVFilesToLocal(List<MVFile> mvFiles) async {
    ...
    List<String> mvFileJsonList = mvFiles
        .map((v) {
          try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<MVFile>>(
        future: _mvFiles,
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            // Filtering logic
            List<MVFile> filteredMVFiles = mvFiles.where((mvFile) {
              if (_searchQuery.isEmpty) return true;
              return mvFile.mvFileNumber.toUpperCase().contains(_searchQuery) ||
                     mvFile.plateNumber!.toUpperCase().contains(_searchQuery);
            }).toList();

            // Pagination
            int startIndex = _currentPage * _rowsPerPage;
            int endIndex = startIndex + _rowsPerPage;
            endIndex = endIndex > filteredMVFiles.length 
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return ListTile(
                        title: Text(mvFile.mvFileNumber),
                        subtitle: Text(mvFile.plateNumber ?? 'N/A'),
                        onTap: () => _showMVFileDetails(mvFile),
                      );
                    },
                  ),
                ),
                ...
                Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: (_currentPage + 1) * _rowsPerPage < filteredMVFiles.length 
                        ? _goToNextPage 
                        : null,
                    ),
                  ],
